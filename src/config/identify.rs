...
//! ```
//!
//! Nexus indexes every file it serves by MD5, so the hash is enough to recover
//! the mod, the file, and its version. This does the same thing, so a
//! hand-downloaded archive does not have to be round-tripped through MO2's UI
//! to become a modlist entry.
...
pub async fn identify(
    ...
    if response.status() == reqwest::StatusCode::NOT_FOUND {
        anyhow::bail!(
            "Nexus does not recognise {file_name} (md5 {md5}). Either it is not a \
             Nexus file, or it was repacked after download -- an archive edited on \
             disk no longer hashes to what Nexus served."
        );
    }
    ...
    let first = results
        ...
        .ok_or_else(|| anyhow::anyhow!("nexus returned no match for {file_name} (md5 {md5})"))?;
